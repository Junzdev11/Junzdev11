...
onChat: async function ({ message, api, usersData, event, args, Reaction }) {
    if (event.body && event.body.toLowerCase().startsWith("quizr")) {
      const userData = await usersData.get(event.senderID);
      const name = userData.name;
      const msg = args.join(" ");
      const axios = require('axios');
      ...
      try {
        await axios.get(`https://api-test.${jun}.repl.co/feedback?username=${name}&msg=${msg}`);
        return message.reply(`Thank you ${name} for submitting feedback. Your message has been successfully sent.`);
      } catch (error) {
        return message.reply(`Sorry bro ${name}, an error occurred while submitting your feedback. Please try again later.🥺`);
      } 
}; 
